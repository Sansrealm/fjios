...
// Helper function to get user session from JWT token (mobile/web)
...
export async function POST(request) {
  try {
    ...
    const {
      ...
      profile_image_url,
      industry_tag_ids,
      // NEW FIELDS: optional location fields
      location_city,
      location_state,
      location_country,
    } = body;
    ...
    // NEW: Generate a clean unique slug from the name (firstname-lastname)
    const baseSlug = name
      .toLowerCase()
      .trim()
      .replace(/[^a-z0-9\s-]/g, "")
    ...
    // Try the base, then add -2, -3, ... until available
